#!/bin/sh
#
# Runs boson-node from a directory holding bin/ and lib/: a Boson distribution, or target/dist.
#
...
exec "$JAVA" $JAVA_OPTS -cp "$BASEDIR/lib/*" io.bosonnetwork.node.cli.BosonNode "$@"
